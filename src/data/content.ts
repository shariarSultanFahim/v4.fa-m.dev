export const DATA = {
  profile: {
    firstName: "Shariar",
    initials: "SF.",
    role: "FULL STACK ENGINEER",
    tagline: "Building scalable solutions that bridge problems and outcomes.",
    description:
      "Full-stack engineer focused on real-world impact, from MVP delivery to production-ready systems.",
    tags: ["React", "Next.js", "ExpressJs", "MongoDB","NestJs", "Prisma","Docker", "Git/GitHub", "Postman", "Cloud Deployment", "Linux Basics"],
    github: "https://github.com/shariarSultanFahim",
    linkedin: "https://www.linkedin.com/in/shariarsultan",
    email: "shariarsfahim@gmail.com",
    phone: "+8801711264984",
  },
  philosophy: {
    title: "Bridge the gap between problems and solutions.",
    bio: "Full-stack engineer experienced in shipping scalable systems that deliver measurable results.",
    detail:
      "I focus on clarity, performance, and speed to value, including shipping MVPs fast and refining toward production quality.",
  },
  branding: {
    title: "Scalable systems, clean delivery, measurable impact.",
    description:
      "I translate complex requirements into reliable, maintainable interfaces and APIs, prioritizing performance, accessibility, and developer efficiency.",
    brands: [
      "React",
      "Next.js",
      "Astro",
      "Tailwind CSS",
      "Express",
      "Docker",
      "MongoDB",
      "Prisma",
    ],
  },
  skills: [
    {
      label: "Frontend",
      items: ["ReactJS", "NextJS", "AstroJs", "TailwindCss"],
    },
    {
      label: "Backend",
      items: ["ExpressJs", "NestJs", "MongoDB", "Prisma ORM", "JWT"],
    },
    {
      label: "Mobile & Desktop",
      items: ["React Native", "ElectronJS"],
    },
    {
      label: "DevOps & Tooling",
      items: [
        "Docker",
        "Git/GitHub",
        "Postman",
        "Cloud Deployment",
        "Linux Basics",
      ],
    },
  ],
  capabilities: [
    {
      title: "Performance Optimization",
      detail:
        "Improved load speed and Lighthouse scores through code splitting, lazy loading, and asset tuning.",
    },
    {
      title: "Design System Delivery",
      detail:
        "Built reusable UI libraries with Tailwind and Styled Components to speed feature delivery.",
    },
    {
      title: "Responsive UI Design",
      detail:
        "Crafted mobile-first interfaces with Tailwind CSS, ensuring optimal layouts across all devices.",
    },
    {
      title: "API Orchestration",
      detail:
        "Implemented TanStack Query patterns for caching, sync, and optimistic UI updates.",
    },
  ],
  experience: [
    {
      role: "Jr. Full Stack Developer",
      company: "Sparktech Agency",
      period: "Feb 2026 - Present",
      bullets: [
        "Bridged the gap between business needs and technical execution by communicating directly with clients to define project scope and requirements.",
        "Built high-performance, SEO-friendly user interfaces using Next.js, ensuring a responsive user experience.",
        "Engineered secure and scalable backend systems using NestJS and Express.js.",
        "Led R&D initiatives to solve complex technical challenges and modernize the tech stack.",
        "Streamlined data flow by effectively managing API integrations between the server and client-side applications.",
      ],
    },
    {
      role: "Frontend Engineer",
      company: "Defined Solution",
      period: "Jul 2025 - Dec 2025",
      bullets: [
        "Accelerated UI development by building a reusable design system, enabling the delivery of a 360-degree virtual tour MVP in just one week.",
        "Boosted load performance by 40% and code reliability by implementing lazy loading, TanStack Query caching, and a full TypeScript migration.",
      ],
    },
    {
      role: "Frontend Engineer Intern",
      company: "Defined Solution",
      period: "Jan 2025 - Jun 2025",
      bullets: [
        "Transformed Figma designs into responsive, accessible React interfaces by building a reusable component library with Tailwind CSS.",
        "Enhanced code maintainability by authoring architectural documentation and standardizing form handling with React Hook Form.",
      ],
    },
  ],
  projects: [
    {
      title: "FixMinCykel.dk",
      tag: "B2B2C",
      desc: "Next.js Frontend for a bike repair service with booking, real-time updates, and performance optimizations.",
      stack: ["Next.js", "Shadcn UI", "Tailwind CSS"],
      link: "https://fixmincykel.dk",
    },
    {
      title: "Hand-n-Handceremonies",
      tag: "Full Stack / E-commerce",
      desc: "Full Stack wedding ceremony booking platform with inventory management, real-time availability, and automated notifications.",
      stack: ["Next.js", "Shadcn UI", "Tailwind CSS", "Prisma ORM"],
      link: "https://hand-n-handceremonies.com",
    },
    {
      title: "Rise & Impact",
      tag: "Frontend / ED Tech",
      desc: "Next.js dashboard for an educational platform with interactive content and performance optimizations.",
      stack: ["Next.js", "Shadcn UI", "Tailwind CSS"],
      link: "https://rise-impact-admin-panel.vercel.app",
    },
    {
      title: "Fahad's Tutorial Shop",
      tag: "Full-Stack / E-commerce",
      desc: "Built a full book store platform with BKash payments and automated delivery workflows via Steadfast APIs.",
      stack: ["Next.js", "Express", "REST APIs"],
      link: "https://shop.ft.education",
    },
    {
      title: "ShareBite",
      tag: "Platform / Social Impact",
      desc: "Donor app + admin dashboard to connect donors, volunteers, and NGOs with automated reporting.",
      stack: ["React", "Dashboard", "Web"],
      link: "https://share-bite-donor-app-hackathon.vercel.app/",
    },
    {
      title: "Retail POS System",
      tag: "Enterprise / Retail",
      desc: "Hardware-software integration for thermal printers and barcode scanners with high-speed UI.",
      stack: ["JavaScript", "Hardware IO", "POS"],
      link: "",
    },
    {
      title: "Garment Production Automation",
      tag: "MVP / Manufacturing",
      desc: "Digitized production stages with inventory state tracking and bottleneck visibility.",
      stack: ["Web", "State Management", "MVP"],
      link: "https://golcas.netlify.app",
    },
    {
      title: "360-Degree Virtual Home Tour",
      tag: "MVP / Real Estate",
      desc: "Interactive property tours with spherical image projection and hotspot navigation.",
      stack: ["PannellumJS", "Web", "MVP"],
      link: "https://360virtualtoureditor.netlify.app",
    },
    {
      title: "2D Overlay Engine",
      tag: "Desktop / Electron",
      desc: "Cross-platform Electron app for transparent anime-style overlays.",
      stack: ["Electron", "Desktop", "Cross-platform"],
      link: "https://github.com/shariarSultanFahim/wifu-engine",
    },
    {
      title: "Hazari Tally",
      tag: "Mobile / Utility",
      desc: "React Native app for digitizing score tracking with localized UI and realtime logic.",
      stack: ["React Native", "Mobile", "Utility"],
      link: "https://github.com/shariarSultanFahim/Hazari-Tally-App",
    },
  ],
  socials: [
    { name: "Email", link: "mailto:shariarsfahim@gmail.com" },
    { name: "GitHub", link: "https://github.com/shariarSultanFahim" },
    { name: "LinkedIn", link: "https://www.linkedin.com/in/shariarsultan" },
  ],
};
